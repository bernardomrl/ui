import { Trigger as me, Close as pe, Title as we, Description as he, Root as ge, Portal as ve, Overlay as ye, Content as be } from "./index.es75.js";
...
function _e() {
  ...
  window.scrollTo(0, 0);
  let h = Bt(st(document, "touchstart", e, {
    passive: !1,
    capture: !0
    ...
    passive: !1,
    capture: !0
  }), st(document, "focus", a, !0), st(window, "scroll", v));
  return () => {
...
function je() {
  const { direction: t, isOpen: n, shouldScaleBackground: e, setBackgroundColorOnScale: o, noBodyStyles: i } = ct(), a = r.useRef(null), v = Ee(() => document.body.style.backgroundColor, []);
  function w() {
...
function Xe({ open: t, onOpenChange: n, children: e, onDrag: o, onRelease: i, snapPoints: a, shouldScaleBackground: v = !1, setBackgroundColorOnScale: w = !0, closeThreshold: T = Fe, scrollLockTimeout: E = We, dismissible: h = !0, handleOnly: f = !1, fadeFromIndex: m = a && a.length - 1, activeSnapPoint: B, setActiveSnapPoint: L, fixed: D, modal: U = !0, onClose: p, nested: M, noBodyStyles: A = !1, direction: c = "bottom", defaultOpen: Y = !1, disablePreventScroll: J = !0, snapToSequentialPoint: u = !1, preventScrollRestoration: b = !1, repositionInputs: y = !0, onAnimationEnd: _, container: P, autoFocus: W = !1 }) {
  ...
  }), [I, q] = r.useState(!1), [X, rt] = r.useState(!1), [Zt, Ct] = r.useState(!1), tt = r.useRef(null), dt = r.useRef(null), ht = r.useRef(null), gt = r.useRef(null), at = r.useRef(null), ot = r.useRef(!1), vt = r.useRef(null), yt = r.useRef(0), et = r.useRef(!1), $t = r.useRef(!Y), At = r.useRef(0), d = r.useRef(null), Mt = r.useRef(((s = d.current) == null ? void 0 : s.getBoundingClientRect().height) || 0), Pt = r.useRef(((F = d.current) == null ? void 0 : F.getBoundingClientRect().width) || 0), bt = r.useRef(0), te = r.useCallback((l) => {
    a && l === it.length - 1 && (dt.current = /* @__PURE__ */ new Date());
  ...
  function ie(l) {
    var S, R;
    !h && !a || d.current && !d.current.contains(l.target) || (Mt.current = ((S = d.current) == null ? void 0 : S.getBoundingClientRect().height) || 0, Pt.current = ((R = d.current) == null ? void 0 : R.getBoundingClientRect().width) || 0, rt(!0), ht.current = /* @__PURE__ */ new Date(), zt() && window.addEventListener("touchend", () => ot.current = !1, {
      once: !0
  ...
  function le(l) {
    if (d.current && X) {
      ...
      let V = k / K;
      const Q = ae(k, g);
      if (Q !== null && (V = Q), x && V >= 1 || !ot.current && !Ht(l.target, g)) return;
      ...
      const G = 1 - V;
      if ((_t || m && nt === m - 1) && (o?.(l, V), $(tt.current, {
        opacity: `${G}`,
        transition: "none"
      }, !0)), N && tt.current && v) {
  ...
  function Nt() {
    if (!d.current) return;
    const l = document.querySelector("[data-vaul-drawer-wrapper]"), S = wt(d.current, c);
    $(d.current, {
  ...
  function ce(l) {
    const S = l ? (window.innerWidth - Z) / window.innerWidth : 1, R = l ? -Z : 0;
